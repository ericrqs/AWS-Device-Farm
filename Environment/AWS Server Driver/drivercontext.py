class InitCommandContext:
    def __init__(self, connectivity, resource):
        assert isinstance(connectivity, ConnectivityContext)
        assert isinstance(resource, ResourceContextDetails)
...
class ResourceCommandContext:
    def __init__(self, connectivity, resource, reservation, connectors):
        assert isinstance(connectivity, ConnectivityContext)
        assert isinstance(resource, ResourceContextDetails)
        assert isinstance(reservation, ReservationContextDetails)
        assert isinstance(connectors, list[Connector])
        self.connectivity = connectivity  # Connectivity details that can help connect to the APIs
        """:type : ConnectivityContext"""
        self.resource = resource  # The details of the resource using the driver
        """:type : ResourceContextDetails"""
...
class ResourceContextDetails:
    def __init__(self, id, name, fullname, type, address, model, family, description, attributes, appdata_json,vmdata_json):
        ...
        """:type : str"""
        assert isinstance(attributes, dict[str, str])
...
class Connector:
    def __init__(self, source, target, target_family, target_model, target_type, target_attributes, direction, alias,attributes, connection_type):
        ...
        """:type : str"""
        assert isinstance(target_attributes, dict[str, str])
...
class ResourceRemoteCommandContext:
    def __init__(self, connectivity, resource, remote_reservation, remote_endpoints):
        assert isinstance(connectivity, ConnectivityContext)
        assert isinstance(resource, ResourceContextDetails)
        assert isinstance(remote_endpoints, list[ResourceContextDetails])
        assert isinstance(remote_reservation, ReservationContextDetails)
        self.connectivity = connectivity  # Connectivity details that can help connect to the APIs
        """:type : ConnectivityContext"""
        self.resource = resource  # The details of the resource using the driver
        """:type : ResourceContextDetails"""
